...
bmap(struct inode *ip, uint bn)
{
  ...
  // double indirect  
    ...
    if((addr = ip->addrs[NDIRECT+1]) == 0) //index 12 addr check
      ip->addrs[NDIRECT+1] = addr = balloc(ip->dev);
   
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;

    if((addr = a[bn/NINDIRECT]) == 0){// block number
    ...
    if(bn < NTPLINDIRECT){
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT+2]) == 0)// index 13
...
itrunc(struct inode *ip)
{
  ...
  if(ip->addrs[NDIRECT+2]){
    ...
    for(w=0; w<NINDIRECT;w++){
        if(a[w]){
            bpp = bread(ip->dev,a[w]);
            aa = (uint*)bpp->data;
            for(m = 0; m<NINDIRECT; m++){
                if(aa[m]){
                    bppp = bread(ip->dev,aa[m]);
                    aaa = (uint*)bppp->data;
